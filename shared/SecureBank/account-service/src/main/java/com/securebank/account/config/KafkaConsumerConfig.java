...
public class KafkaConsumerConfig {
    ...
    /**
     * Common Avro consumer settings shared by both listeners.
     *
     * Deserializers are wrapped in ErrorHandlingDeserializer so deserialization failures
     * (e.g., schema-incompatible payload) reach DefaultErrorHandler and get routed to the
     * DLT instead of crashing the consumer.
     */
    private Map<String, Object> baseConsumerProps(String consumerGroupId) {
        ...
        // Return generated SpecificRecord types (com.securebank.avro.*) instead of GenericRecord.
    ...
    public ConsumerFactory<String, Alert> alertConsumerFactory() {
        // Separate group so alert consumption progresses independently of transaction consumption.
        return new DefaultKafkaConsumerFactory<>(baseConsumerProps(groupId + "-alerts"));
    }
    ...
    public DefaultErrorHandler kafkaErrorHandler(KafkaTemplate<Object, Object> dltKafkaTemplate) {
        // Derive the DLT from the source topic (transactions → transactions.DLT, alerts → alerts.DLT)
        // and keep the original partition so per-account ordering is preserved on the DLT.
