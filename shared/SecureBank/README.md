...
```
  ...
  └────────┘                            │  (REST + Producer)  │
              ...
              │  → PostgreSQL      │                       │   4 fraud rules         │
              └─────────▲──────────┘                       └────────────┬────────────┘
                        │ freezes account                               │ rule fires
                        │ (status = FROZEN)                             ▼
                        │                                     ┌──────────────────┐
                        └──────────────────────────────────── │  Topic: alerts   │
                                                              └──────────────────┘
...
**Fraud rules:** HIGH_VELOCITY (>5 txns/60s window), LARGE_AMOUNT (single txn > 100k), SPEND_SPIKE (>5× rolling average), GEO_IMPOSSIBLE (consecutive cities requiring >900 km/h travel).

**Closed loop:** account-service also consumes `alerts` and freezes the account (single-writer principle — only account-service touches the `accounts` table). Frozen accounts reject WITHDRAW/TRANSFER (routed to the DLT for audit); deposits stay allowed.
...
The alert also **freezes the account**: `account-service` consumes `alerts` and sets `status = FROZEN`. Verify:
```bash
docker exec -it securebank-postgres psql -U bank -d securebank \
  -c "SELECT account_id, status FROM accounts;"
```
Any further WITHDRAW/TRANSFER for that account is rejected and lands in `transactions.DLT` (deposits still work).

### 7. Trigger a geo-impossible alert
Send two transactions from cities that can't be reached in time:
```bash
curl -X POST http://localhost:8081/api/transactions \
  -H "Content-Type: application/json" \
  -d '{"accountId": "ACC1003", "type": "WITHDRAW", "amount": 100, "city": "Mumbai"}'

curl -X POST http://localhost:8081/api/transactions \
  -H "Content-Type: application/json" \
  -d '{"accountId": "ACC1003", "type": "WITHDRAW", "amount": 100, "city": "London"}'
```
The second one requires faster-than-900 km/h travel → **GEO_IMPOSSIBLE** alert → account frozen.
...
| Late-event handling | 10s grace period on the tumbling window |
| Stateful stream processing | State stores: rolling spend average + last-seen location |
| Event-driven feedback loop | `alerts` consumed back by account-service → account freeze |
